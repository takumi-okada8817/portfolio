<?php
    ...
    if(isset($_SESSION['name'])){
        ...
        try{
            $sql = "select username,userid,uniqid from users where identify=:userid";
        ...
        try{
            if(isset($_POST['newun'])){
                ...
                if($newun === ""){
                    $error_message = space_check($newun);
                    throw new Exception("$error_message");
                }

                $sql = "update users set username=:newun where identify=:userid";
                $stmt = $dbh->prepare($sql);
                $stmt->bindValue(':newun',$newun);
                $stmt->bindValue(':userid',$userid);
                $stmt->execute();

                $sql = "update posttext set username=:newun where identify=:userid";
                $stmt = $dbh->prepare($sql);
                $stmt->bindValue(':newun',$newun);
                $stmt->bindValue(':userid',$userid);
                $stmt->execute();

                echo "<script>alert('ユーザーネームの変更完了');</script>";
...
<html lang="ja">
    ...
    <body>
        ...
        <div class="wrapper">
            <main id="homemain">
                <p>ユーザーネーム : <?= $user_name['username'] ?></p>
                <form action="usernamechg.php" method="POST">
                    <input type="text" name="newun">
